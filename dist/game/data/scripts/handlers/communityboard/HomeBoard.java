...
public class HomeBoard implements IParseBoardHandler
{
	...
	public boolean parseCommunityBoardCommand(String command, Player player)
	{
		...
		if (returnHtml != null)
		{
			if (Config.CUSTOM_CB_ENABLED)
			{
				...
				returnHtml = returnHtml.replace("%onlinecount%", Integer.toString(World.getInstance().getPlayers().size()));
				returnHtml = returnHtml.replaceAll("%playerpremium%", player.hasPremiumStatus() ? "<font color=\"LEVEL\">高級用戶</font>" : "<font color=\"7A8B8B\">普通用戶</font>");
				returnHtml = returnHtml.replaceAll("%onlinetime%", (new SimpleDateFormat("HH:mm").format(player.getUptime() - 28800000)));
				returnHtml = returnHtml.replaceAll("%playerclan%", String.valueOf(player.getClan() == null ? "<font color=\"7A8B8B\">無血盟</font>" : player.getClan().getName()));
				returnHtml = returnHtml.replaceAll("%playerclass%", ClassListData.getInstance().getClass(player.getClassId()).getClassName());
				returnHtml = returnHtml.replaceAll("%playerlevel%", String.valueOf(player.getLevel()));
				returnHtml = returnHtml.replaceAll("%serverUpTime%", getServerUpTime());
			}
	...
	{
		long time = GameServer.dateTimeServerStarted.getTimeInMillis();

		final long days = TimeUnit.MILLISECONDS.toDays(time);
		time -= TimeUnit.DAYS.toMillis(days);
		final long hours = TimeUnit.MILLISECONDS.toHours(time);
		time -= TimeUnit.HOURS.toMillis(hours);
		return days + ":天," + hours + ":時," + TimeUnit.MILLISECONDS.toMinutes(time) + ":分";
	}
	/**
